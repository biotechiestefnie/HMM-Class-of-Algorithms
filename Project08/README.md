...
In probability algorithms such as HMM, Viterbi, and Forward/Backward, the application of natural log is standard because it is mathematically easier, computationally faster, and numerically more stable for very small probabilities. In fact, natural log is the default log in Numpy and most other scientific libraries, with np.log(x) computing ln(x) and requiring an implementation of np.log10(x) to calculate log base 10. When probabilities are converted to natural log, all valuies in the interval range of 0 < p < 1 become negative numbers, with smaller original probabilities becoming more negative log values. If we think of the log probabilities as penalties, every transition and emission adds a penalty, leading to a more negative value. The Viterbi algorithm thus chooses the path with the smallest penalty, which corresponds to the largest probability and the least negative log value as the most optimal path. 
